...
const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // SYNC: Cargar overrides desde el servidor
  useEffect(() => {
  ...
  // DOLAR BLUE: Default 1200, but fetched from API. Adjustable by Admin.
  ...
  // Fetch Dolar only once on mount, then allow overrides
  useEffect(() => {
    const fetchDolar = async () => {
      try {
        ...
        if (data && data.venta) {
          console.log("Dolar Blue API:", data.venta);
  ...
  // --- LOGICA DE PRECIOS (CORREGIDA SEGÚN REQUERIMIENTO) ---
  // Formula: (Costo USD * Dolar Blue) + (Margen %)
  // Matemáticamente: (Costo * Blue) * (1 + Margen/100)
  ...
  const bulkUpdateMargins = async (type: 'retail' | 'wholesale', value: number) => {
    ...
    try {
        ...
        showAlert("Error", "Falló la actualización masiva en el servidor. Revise la consola (F12) para más detalles.", "error");
    }
  };
...
// ... (VideoHero, ProductGridItem, ProductListItem, PerkinsChatModal, ProductModal, CartDrawer kept as is) ...
...
const PerkinsChatModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const handleSend = async () => {
    ...
    const response = await sendMessageToPerkins(`Contexto: Chat General. Pregunta: ${userMsg}`, dolarBlue, products);
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 animate-fade-in">
      ...
      <div className="relative bg-luxury-card w-full max-w-4xl h-[90vh] rounded-2xl border border-gold-600/50 shadow-[0_0_50px_rgba(212,175,55,0.15)] flex flex-col md:flex-row overflow-hidden animate-slide-up">
        ...
        <div className="w-full md:w-1/2 bg-black relative flex items-end justify-center overflow-hidden border-b md:border-b-0 md:border-r border-gold-600/20">
            ...
            <img src={PERKINS_IMAGES.HOLA} className="h-full w-auto object-cover object-top md:object-center transform scale-110" alt="Mr. Perkins"/>
        ...
        <div className="w-full md:w-1/2 flex flex-col bg-neutral-900">
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 space-y-6 bg-neutral-900/30">
                ...
                {isTyping && <div className="flex items-center gap-2 text-gold-500/50 text-xs ml-4"><Loader2 size={12} className="animate-spin" /><span>Mr. Perkins está pensando...</span></div>}
            </div>
            <div className="p-4 bg-black border-t border-gold-600/20">
                <div className="flex gap-2 bg-neutral-900/50 border border-neutral-700 rounded-full p-1 pl-4 focus-within:border-gold-500 transition-colors">
                    <input type="text" value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSend()} placeholder="Escriba su consulta aquí..." className="flex-1 bg-transparent text-white text-sm outline-none placeholder-gray-600" />
...
const ProductModal: React.FC<{ product: Product | null, onClose: () => void }> = ({ product, onClose }) => {
  const { addToCart, cart, decreaseFromCart, calculateFinalPriceARS, formatPrice, dolarBlue, showAlert, products } = useStore();
  const [chatInput, setChatInput] = useState("");
  const [chatHistory, setChatHistory] = useState<ChatMessage[]>([]);
  const [isTyping, setIsTyping] = useState(false);
  const scrollRef = useRef<HTMLDivElement>(null);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in">
      ...
      <div className="relative bg-luxury-card w-full max-w-4xl h-[90vh] md:h-auto md:max-h-[90vh] rounded-2xl border border-gold-600/30 shadow-2xl flex flex-col md:flex-row overflow-hidden">
        <button onClick={onClose} className="absolute top-4 right-4 z-20 text-gray-400 hover:text-white bg-black/50 rounded-full p-2"><X size={24} /></button>
        <div className="w-full md:w-1/2 bg-black relative"><img src={product.image} className="w-full h-full object-contain p-8" alt={product.nombre} /></div>
        <div className="w-full md:w-1/2 flex flex-col bg-neutral-900">
            <div className="p-6 md:p-8 border-b border-neutral-800 flex-shrink-0">
                ...
                <div className="flex flex-wrap gap-2 mb-6">{product.tags_olfativos.map(tag => (<span key={tag} className="text-xs bg-neutral-800 text-gray-300 px-2 py-1 rounded border border-neutral-700 capitalize">{tag}</span>))}</div>
            ...
            <div className="flex-1 flex flex-col min-h-0 bg-black/30">
                <div className="p-3 bg-gold-900/20 border-b border-gold-600/10 flex items-center gap-3"><div className="w-8 h-8 rounded-full border border-gold-500/50 overflow-hidden"><img src={PERKINS_IMAGES.LOGO} className="w-full h-full object-cover"/></div><span className="text-gold-500 text-sm font-bold">Consultar a Mr. Perkins</span></div>
                <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4">
                    {chatHistory.map((msg, idx) => (
                        <div key={idx} className={`flex ${msg.role === ChatRole.USER ? 'justify-end' : 'justify-start'}`}>
                            <div className={`max-w-[85%] rounded-lg p-3 text-sm ${msg.role === ChatRole.USER ? 'bg-neutral-800 text-white' : 'bg-gold-900/20 text-gold-100 border border-gold-600/20'}`}>{msg.text}</div>
                        </div>
                    ))}
                    {isTyping && <div className="text-xs text-gray-500 ml-2 animate-pulse">Mr. Perkins está escribiendo...</div>}
                </div>
                <div className="p-4 border-t border-neutral-800 flex gap-2">
                    <input type="text" value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSend()} placeholder="Pregunte sobre notas, ocasión, etc..." className="flex-1 bg-neutral-900 border border-neutral-700 rounded-full px-4 py-2 text-sm text-white focus:border-gold-500 outline-none" />
...
const CartDrawer: React.FC = () => {
    ...
    const handleCheckout = async () => {
        if (!customerInfo.name || !customerInfo.address || !customerInfo.date) { alert("Por favor complete todos los datos de envío."); return; }
        ...
        } catch (error) { console.error(error); alert("Error al procesar el pago."); } finally { setIsCheckingOut(false); }
    };
    ...
    return (
        <div className="fixed inset-0 z-[60] flex justify-end">
            ...
            <div className="relative w-full max-w-md bg-neutral-900 h-full shadow-2xl flex flex-col animate-slide-left border-l border-gold-600/20">
                <div className="p-6 border-b border-neutral-800 flex items-center justify-between bg-black"><h2 className="text-xl font-serif text-white">Su Selección</h2><button onClick={() => setIsCartOpen(false)} className="text-gray-400 hover:text-white"><X size={24} /></button></div>
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    {cart.length === 0 ? <div className="text-center text-gray-500 mt-20"><ShoppingBag size={48} className="mx-auto mb-4 opacity-20" /><p>Su carrito está vacío.</p></div> : (
                        <>
                            <div className="space-y-4">
                                {cart.map(item => (
                                    <div key={item.id} className="flex gap-4 bg-black/40 p-3 rounded-lg border border-neutral-800">
                                        <div className="w-16 h-16 bg-white/5 rounded overflow-hidden flex-shrink-0"><img src={item.image} className="w-full h-full object-cover" /></div>
                                        <div className="flex-1">
                                            <div className="flex justify-between mb-1"><h4 className="font-medium text-white text-sm line-clamp-1">{item.nombre}</h4><button onClick={() => removeFromCart(item.id)} className="text-gray-500 hover:text-red-500"><X size={14} /></button></div>
                                            <p className="text-xs text-gray-400 mb-2">{item.marca}</p>
                                            <div className="flex items-center justify-between"><div className="text-gold-500 font-bold text-sm">{formatPrice(calculateFinalPriceARS(item) * item.quantity)}</div><QuantityControl compact product={item} quantityInCart={item.quantity} onAdd={() => addToCart(item)} onRemove={() => decreaseFromCart(item)} /></div>
                                        </div>
                                    </div>
                                ))}
                            </div>
                            <div className="border-t border-neutral-800 pt-6">
                                <div className="flex justify-between text-xl font-serif text-white mb-6"><span>Total Estimado</span><span className="text-gold-500">{formatPrice(total)}</span></div>
                                <div className="space-y-4 mb-6"><h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider">Datos de Entrega</h3><input type="text" placeholder="Nombre Completo" className="w-full bg-black border border-neutral-700 p-3 rounded text-white text-sm outline-none focus:border-gold-600" value={customerInfo.name} onChange={e => setCustomerInfo({...customerInfo, name: e.target.value})} /><input type="text" placeholder="Dirección de Envío" className="w-full bg-black border border-neutral-700 p-3 rounded text-white text-sm outline-none focus:border-gold-600" value={customerInfo.address} onChange={e => setCustomerInfo({...customerInfo, address: e.target.value})} /><input type="date" className="w-full bg-black border border-neutral-700 p-3 rounded text-white text-sm outline-none focus:border-gold-600" value={customerInfo.date} onChange={e => setCustomerInfo({...customerInfo, date: e.target.value})} /></div>
                            </div>
                        </>
                    )}
                </div>
                {cart.length > 0 && <div className="p-6 border-t border-neutral-800 bg-black"><button onClick={handleCheckout} disabled={isCheckingOut} className="w-full bg-gold-600 hover:bg-gold-500 text-black font-bold py-4 rounded-lg uppercase tracking-widest flex items-center justify-center gap-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">{isCheckingOut ? <Loader2 className="animate-spin" /> : <CreditCard size={20} />} Finalizar Compra</button></div>}
            </div>
        </div>
    );
};
  ...
  return (
    <div className="min-h-screen bg-neutral-900 pb-20 flex flex-col">
       ...
       <div className="container mx-auto px-4 py-8 relative z-10 -mt-10 flex-1">
          ...
          {filteredProducts.length === 0 && <div className="text-center py-20"><p className="text-gray-500 text-lg">No se encontraron productos con los filtros seleccionados.</p><button onClick={() => window.location.reload()} className="mt-4 text-gold-500 hover:underline">Limpiar Filtros</button></div>}
       </div>
       <div className="fixed bottom-2 right-4 z-50 animate-slide-up"><button onClick={() => setIsPerkinsChatOpen(true)} className="w-16 h-16 rounded-full border-2 border-gold-500 shadow-[0_0_30px_rgba(212,175,55,0.5)] overflow-hidden hover:scale-110 transition-transform duration-300 relative group bg-black"><img src={PERKINS_IMAGES.LOGO} className="w-full h-full object-cover" /><div className="absolute inset-0 bg-black/10 group-hover:bg-transparent transition-colors" /></button></div>
       <FloatingPricingBar />
       <CartDrawer />
  ...
  return (
    <div className="min-h-screen bg-neutral-900 text-gray-200 flex">
      ...
      <main className="flex-1 p-8 ml-64 md:ml-0 overflow-y-auto">
                ...
                <div className="bg-neutral-800/30 border border-neutral-700 p-6 rounded-lg mb-6">
                    ...
                    <div className="flex flex-col md:flex-row gap-6">
                        ...
                        <div className="flex-1 bg-black/50 p-4 rounded border border-neutral-800">
                            ...
                            <div className="flex gap-2"><input type="number" value={globalWholesale} onChange={(e) => setGlobalWholesale(Number(e.target.value))} className="bg-neutral-900 border border-neutral-700 rounded p-2 text-white w-20 text-center" /><button onClick={() => bulkUpdateMargins('wholesale', globalWholesale)} className="bg-gold-600 text-black px-4 rounded font-bold hover:bg-gold-500 text-sm">Aplicar a Todos</button></div>
                        </div>
                    </div>
                </div>
             )}
               ...
               <table className="w-full text-left text-sm whitespace-nowrap">
                   ...
                   <tr>
                     ...
                     {currentUser.role === 'admin' && (
                         <>
                            ...
                            <th className="p-4 text-center bg-blue-900/10">Precio Whsle</th>
                         </>
                     )}
                 ...
                 <tbody className="divide-y divide-neutral-800">
                   {filteredInventory.map(product => {
                       ...
                       const wholesalePrice = costoARS * (1 + (product.margin_wholesale || 15)/100);
                       ...
                       return (
                         <tr key={product.id} className="hover:bg-neutral-900/30 transition-colors group">
                           ...
                           {currentUser.role === 'admin' ? (
                               <>
                                    <td className="p-4 text-center"><input type="number" value={product.precio_usd} onChange={(e) => updateProduct(product.id, { precio_usd: Number(e.target.value) })} className="w-16 bg-neutral-900 border border-neutral-700 rounded px-2 py-1 text-center text-white outline-none focus:border-gold-500" /></td>
                                    <td className="p-4 text-center text-gray-600 font-mono text-xs">{formatPrice(costoARS)}</td>
                                    
                                    <td className="p-4 text-center bg-neutral-800/30"><div className="flex items-center justify-center gap-1"><input type="number" value={product.margin_retail || 50} onChange={(e) => updateProduct(product.id, { margin_retail: Number(e.target.value) })} className="w-14 bg-neutral-900 border border-neutral-700 rounded px-1 py-1 text-center text-green-400 font-bold outline-none focus:border-green-500" /><span className="text-xs text-gray-500">%</span></div></td>
                                    <td className="p-4 text-center bg-neutral-800/30 text-green-400 font-bold">{formatPrice(retailPrice)}</td>
                                    
                                    <td className="p-4 text-center bg-blue-900/10"><div className="flex items-center justify-center gap-1"><input type="number" value={product.margin_wholesale || 15} onChange={(e) => updateProduct(product.id, { margin_wholesale: Number(e.target.value) })} className="w-14 bg-neutral-900 border border-neutral-700 rounded px-1 py-1 text-center text-blue-400 font-bold outline-none focus:border-blue-500" /><span className="text-xs text-gray-500">%</span></div></td>
                                    <td className="p-4 text-center bg-blue-900/10 text-blue-400 font-bold">{formatPrice(wholesalePrice)}</td>
                                    
                                    <td className="p-4 text-center"><input type="number" value={product.stock} onChange={(e) => updateProduct(product.id, { stock: Number(e.target.value) })} className={`w-14 bg-neutral-900 border rounded px-1 py-1 text-center font-bold outline-none ${product.stock === 0 ? 'border-red-900 text-red-500' : 'border-neutral-700 text-white'}`} /></td>
                               </>
                           ) : (
                               <>
        ...
        {/* User Management code remains same but hidden here for brevity since no changes needed there */}
